...
May 31, 2023 | Sakib Rasul

CORE DELIVERABLES
The user should be able to...
1. View images for each ramen in #ramen-menu on http://localhost:3000/ramens on load.
2. Click on an image and have its details populated in #ramen-detail.
3. Create a new ramen in #ramen-menu by submitting the form in #new-ramen.
*/
...
fetch("http://localhost:3000/ramens")
    ...
    .then(ramenDishes=> { ramenDishes.forEach(ramen => loadRamen(ramen)) });
...
document.querySelector("#new-ramen").addEventListener("submit", event => {
    ...
    // Instantiate a new ramen dish and pass it to `loadRamen`.
    loadRamen({
